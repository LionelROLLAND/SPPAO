...
list<infoPath>* secondSPPAO_cstarPlus(list<Node*>& graph, Node* s, Node* t, int* n1, int* n2,
double* t1, double* t2) {
    //cout<<"\nInto secondSPPAO"<<endl;
    int nbD1 = 0;
    int step = 0;
    ...
    t->pred = new arcNode(nullptr, inf, 0);
    ...
    while(!criteriaSpace.empty()) {
        ...
        if (way) {
            upper = dijkstraOptiCD_condCstarD_step(s, t, d_bar, Irect.c_max);
        } else {
            upper = revDijkstraOptiCD_condCstarD_step(s, t, d_bar, Irect.c_max);
        }
        way = !way;
        ...
        if (upper.path->size() > 1) {
            ...
            if (upper.c == Irect.pathMin->c) {
                ...
                if (way) {
                    lower = dijkstraOptiCD_condCstarD_step(s, t, Irect.pathMin->d, upper.c);
                } else {
                    lower = revDijkstraOptiCD_condCstarD_step(s, t, Irect.pathMin->d, upper.c);
                }
                way = !way;
...
double* t1, double* t2) {
    ...
    while(!criteriaSpace.empty()) {
        ...
        criteriaSpace.erase(criteriaSpace.begin());
        c_bar = (Irect.c_max + Irect.pathMin->c)/2;
        ...
        if (way) {
            left = dijkstraOptiCD_condCstarD_step(s, t, Irect.pathMin->d, c_bar);
        } else {
            left = revDijkstraOptiCD_condCstarD_step(s, t, Irect.pathMin->d, c_bar);
        }
        way = !way;
        ...
        if (t1 != nullptr) {*t1 += elapsed.count();}
        if (n1 != nullptr) {(*n1)++;}
        if (logs) {cout<<"result : d = "<<left.d<<", c = "<<left.c<<"\n";}

        if (left.path->size() > 1) {

            if (logs) {
                cout<<"Adding path : d = "<<left.d<<", c = "<<left.c<<"\n";
                logStream<<"point "<<step+1<<" "<<left.c<<" "<<left.d<<" ";
                logStream<<(int) PSr<<" "<<(int) PSg<<" "<<(int) PSb<<"\n";
            }
            res->push_front(left);

            if (left.c == Irect.pathMin->c) {
            ...
            if (left.d == d_max) {
                is_dmax_reached = true;
            } else {
                if (logs) {
                    cout<<"Adding rectangle : bottom = "<<res->begin()->d<<", top = ";
                    cout<<Irect.d_max<<", left = "<<res->begin()->c<<", right = "<<Irect.c_max<<"\n";
                }
                criteriaSpace.push_front(Rectangle({Irect.pathMin, left.c, left.d, step+1}));

                //resetGraph(graph);
                //simpleResetGraph(graph);

                if (logs) {
                    cout<<"#"<<++nbD1<<" SPPAO2 -- Dijkstra, lower, strict_min_d = "<<left.d;
                    cout<<", strict_max_c = "<<Irect.c_max<<"\n";
                }
                ...
                if (way) {
                    right = dijkstraOptiCD_condCstarD_step(s, t, left.d, Irect.c_max);
                } else {
                    right = revDijkstraOptiCD_condCstarD_step(s, t, left.d, Irect.c_max);
                }
                way = !way;
                ...
                if (right.path->size() > 1) {
                    ...
                    if (logs) {
                        logStream<<"rect "<<step+1<<" "<<step+2<<" "<<right.d<<" ";
                        logStream<<Irect.d_max<<" "<<right.c<<" "<<Irect.c_max<<" ";
                        logStream<<(int) RNr<<" "<<(int) RNg<<" "<<(int) RNb<<"\n";
                    }
                }
            }
